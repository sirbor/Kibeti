...
struct ReqPay: View {
    ...
    @ObservedObject var loanBalance = KibetiBalance.instance
    ...
    var body: some View {
        NavigationStack {
                ...
                VStack(spacing: 20) {
                    ...
                    if reqPay == .loan {
                        ...
                        Text("Loan Amount: KSH. \(String(format: "%.2f", loanBalance.loanBalance))")
                            .fontWeight(.bold)
                            .foregroundStyle(homeVM.kibetiBalance  < loanAmount ? Color.primary : Color.orange)
                            .font(.caption)
                    }
                    HStack {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 25.0)
                            .fill(loanAmount <= 10 ? .black.opacity(0.3) : .kibetiGreen)
                            .frame(height: 55)
                    )
                    .padding(.top, 40)
                    .onTapGesture {
            ...
            .navigationDestination(isPresented: $navigateToLoan) {
                ConfirmReqPay(reqPayPath: .loan, loanAmount: loanAmount,  isKibeti: true)
