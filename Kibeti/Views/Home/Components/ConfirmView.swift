...
//                Fuliza(fulizaAmount:  amount - homeVM.kibetiBalance, cancel: $showFuliza)
...
//            }
    ...
    func showFulizaAlert() -> Bool {
        if amount > homeVM.kibetiBalance {
            return true
        } else {
            return false
        }
            ...
            .overlay {
                VStack(spacing: 12) {
                    Text("SEND MONEY USING FULIZA")
                        .fontWeight(.semibold)
                    Text("You have insufficient funds on Kibeti. You are about to ")
                        .fontWeight(.light)
                    +
